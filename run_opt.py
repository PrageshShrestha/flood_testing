...
Fixed: video output dimensions (resize to original)
Fixed: bounding box coordinate transformation (region → original frame)
"""
...
class Config:
    ...
    resize_input: float = 0.5          # resize full frame before cropping
    region_scale: float = 0.5          # resize each region before inference
    conf: float = 0.3
    iou: float = 0.45
...
class Stage1:
    ...
    def run(self, video_path: str):
        ...
        print(f"Input: {orig_width}x{orig_height}, {total} frames, {fps:.1f}fps")
        ...
        def process_frame(frame_data):
            idx, frame_orig = frame_data
            t0 = time.perf_counter()
            
            try:
                # Step 1: resize full frame
                if self.cfg.resize_input < 1.0:
                    h, w = frame_orig.shape[:2]
                    frame = cv2.resize(frame_orig, (int(w * self.cfg.resize_input), int(h * self.cfg.resize_input)))
                else:
                    frame = frame_orig
                
                h, w = frame.shape[:2]   # resized dimensions
                regions = []
                ...
                for rid, (coords, name) in enumerate(zip(self.cfg.regions, self.cfg.region_names)):
                    ...
                    # crop from resized frame
                    region_crop = frame[y1:y2, x1:x2]
                    
                    # further scale region for inference
                    if self.cfg.region_scale < 1.0:
                        region_small = cv2.resize(region_crop, 
                            (int(region_crop.shape[1] * self.cfg.region_scale),
                             int(region_crop.shape[0] * self.cfg.region_scale)))
                    else:
                        region_small = region_crop
                    
                    regions.append({
                        'rid': rid,
                        'name': name,
                        'data': region_small,          # the image that will be sent to inference
                        'crop_rect': (x1, y1, x2, y2), # in resized-frame coordinates
                    })
                ...
                out_queue.put({
                    'idx': idx,
                    'regions': regions,
                    'frame': frame,                    # resized full frame (for drawing)
                    'resized_size': (w, h),
        ...
        # Return also original dimensions for FFmpeg
        return out_queue, total, fps, orig_width, orig_height
...
class Stage2:
    ...
    def infer_region(self, region_img, name, idx):
        t0 = time.perf_counter()
        try:
            results = self.model(region_img, verbose=False)[0]
        ...
        while self.running:
            try:
                ...
                out_queue.put({
                    'idx': prepped['idx'],
                    'frame': prepped['frame'],
                    'resized_size': prepped['resized_size'],
                    'orig_size': prepped['orig_size'],
                    'preprocess_ms': prepped.get('preprocess_ms', 0),
                    'regions': regions,           # keep region metadata (crop_rect)
                    'results': results,
                    'inference_ms': inf_ms,
                    'tl_ms': tl_ms,
                    'tr_ms': tr_ms,
...
class Stage3:
    ...
    def run(self, in_queue: queue.Queue, fps: float, orig_width: int, orig_height: int,
            run_dir: Path, metrics: MetricsCollector):
        print("\n[STAGE3] POSTPROCESSING")
        print("-"*50)
        
        # Video writer – expects original dimensions
        if self.cfg.save_video:
            video_path = run_dir / 'output.mp4'
            cmd = ['ffmpeg', '-y', '-f', 'rawvideo', '-vcodec', 'rawvideo',
                   '-pix_fmt', 'bgr24', '-s', f'{orig_width}x{orig_height}', '-r', str(fps),
        ...
        while self.running:
            try:
                ...
                # Transform all detections from region-scaled coordinates to resized-frame coordinates
                resized_w, resized_h = data['resized_size']
                scale_to_orig_x = orig_width / resized_w
                scale_to_orig_y = orig_height / resized_h
                
                all_boxes = []
                all_scores = []
                all_classes = []
                
                for region_info, region_result in zip(data['regions'], data['results']):
                    crop_x1, crop_y1, crop_x2, crop_y2 = region_info['crop_rect']
                    # boxes are in region_small coordinates (after region_scale)
                    for i in range(len(region_result['boxes'])):
                        # Box in region_small space
                        x1_small, y1_small, x2_small, y2_small = region_result['boxes'][i]
                        # Scale up to crop region (undo region_scale)
                        inv_scale = 1.0 / self.cfg.region_scale
                        x1_crop = x1_small * inv_scale
                        y1_crop = y1_small * inv_scale
                        x2_crop = x2_small * inv_scale
                        y2_crop = y2_small * inv_scale
                        # Translate to resized-frame coordinates
                        x1_resized = crop_x1 + x1_crop
                        y1_resized = crop_y1 + y1_crop
                        x2_resized = crop_x1 + x2_crop
                        y2_resized = crop_y1 + y2_crop
                        # Clip to resized frame bounds
                        x1_resized = max(0, min(resized_w, x1_resized))
                        y1_resized = max(0, min(resized_h, y1_resized))
                        x2_resized = max(0, min(resized_w, x2_resized))
                        y2_resized = max(0, min(resized_h, y2_resized))
                        
                        all_boxes.append([x1_resized, y1_resized, x2_resized, y2_resized])
                        all_scores.append(region_result['scores'][i])
                        all_classes.append(region_result['classes'][i])
                
                # Apply NMS on resized-frame coordinates
                if all_boxes:
                ...
                # Resize annotated frame to original dimensions for video output
                if annotated.shape[1] != orig_width or annotated.shape[0] != orig_height:
                    annotated_full = cv2.resize(annotated, (orig_width, orig_height))
                else:
                ...
                if self.cfg.save_video and self.video_process:
                    try:
                        self.video_process.stdin.write(annotated_full.tobytes())
                ...
                frame_results.append({
                    'frame': data['idx'],
                    'detections': len(boxes),
                    'boxes': boxes,          # in resized-frame coordinates (for reference)
                    'scores': scores,
                    'classes': classes
                })
...
class Pipeline:
    ...
    def run(self):
        ...
        try:
            q1, total, fps, w_orig, h_orig = self.stage1.run(self.cfg.video_path)
            print(f"\n✅ Stage1 complete, {total} frames preprocessed")
            
            q2 = self.stage2.run(q1)
            print(f"✅ Stage2 complete")
            
            results = self.stage3.run(q2, fps, w_orig, h_orig, self.run_dir, self.metrics)
